...
H = fft2(h, conv_size, conv_size) + 1e-12;
...
psd_est = [linspace(1, conv_size/2, conv_size/2) linspace(conv_size/2, 1, conv_size/2)] / (conv_size/2);
psd_est = psd_est'.^2 * psd_est.^2;

x_hat_wiener_est = abs(ifft2((conj(psd_h) ./ (psd_h + psd_est)) .* (Y_noisy ./ H)));
x_hat_wiener_est = imcrop(x_hat_wiener_est, rect_crop);

% Calculate PSNR and SSIM of X_Hat Wiener Estimated
psnr_x_wiener_est = PSNR(x, x_hat_wiener_est, R);
[ssim_x_wiener_est, ~] = ssim(x_hat_wiener_est, x);
